...
const Chatbot = () => {
  ...
  const API_KEY = process.env.REACT_APP_GEMINI_API_KEY || "AIzaSyCD3DlFj2PzoDW5YXI9P6IJ8521-ecSUfw";
  ...
  const sendMessage = async (voiceText = null) => {
    ...
    try {
      ...
      // En la función sendMessage, reemplaza esta sección:
      if (isDemoRequest && demoData) {
        ...
        // Preparar instrucciones para Gemini basadas en el análisis ya hecho
        ...
        try {
          // Enviar a Gemini solo para reformular el análisis
      ...
      // Si no es una solicitud de datos de demo, continuar con el flujo normal
      let response;
      const headers = {
        "Content-Type": "application/json"
      };
      
      // Modify your systemPrompt in the sendMessage function
      // Replace your current systemPrompt with this improved version
      const systemPrompt = `Eres un analista ambiental experto. Tu objetivo es analizar datos de calidad del aire y proporcionar insights útiles.
      ...
      if (imageFile) {
        ...
        const data = {
          contents: [{
            parts: [
              { text: systemPrompt + " " + (textToSend || "Analiza esta imagen") },
              { 
                inline_data: { 
        ...
        response = await axios.post(IMAGE_URL, data, { 
          headers,
          timeout: 30000
        });
      } else {
        ...
        const data = {
          contents: [{
            parts: [{text: systemPrompt + " " + textToSend}]
          }]
        };
